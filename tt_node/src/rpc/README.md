...
### 1. `rpc_server.rs` - HTTP RPC (⚠️ INSECURE)

**TYLKO DO UŻYTKU LOKALNEGO!**

```rust
use tt_node::rpc::RpcServer;

let server = RpcServer::new(8080, node_id, is_validator, node);
server.start().await?;
```

- ❌ Brak szyfrowania (plaintext HTTP)
- ⚠️ Używaj TYLKO dla localhost
- ⚠️ Nie używaj w produkcji

### 2. `rpc_secure.rs` - PQ Secure RPC (✅ PRODUKCJA)
...
**PAMIĘTAJ**: Zawsze używaj `SecureRpcServer` w produkcji. `RpcServer` (HTTP) to tylko development tool!
