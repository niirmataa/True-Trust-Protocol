...
#[derive(Debug, Clone)]
#[allow(dead_code)] // Fields used for session management and debugging
struct SessionState {
    session_id: [u8; 32],
    created_at: Instant,
...
pub struct SecureRpcServer {
    ...
    /// Next identity for staged key rotation (announced before becoming active)
    next_identity: Arc<RwLock<Option<NodeIdentity>>>,
    /// Timestamp when next_identity becomes active
    next_identity_active_at: Arc<RwLock<Option<u64>>>,
}
...
impl SecureRpcServer {
    ...
    fn new_with_options(
        ...
        Self {
            ...
            next_identity: Arc::new(RwLock::new(None)),
            next_identity_active_at: Arc::new(RwLock::new(None)),
        }
    }
    ...
    async fn handle_connection(&self, mut stream: TcpStream, addr: SocketAddr) -> Result<()> {
        ...
        let last_renegotiation = Instant::now();
        ...
        loop {
            ...
            // Key renegotiation check - refresh session keys when nonce counter is high
            if last_renegotiation.elapsed() > MIN_RENEGOTIATION_INTERVAL
                && channel.should_renegotiate()
            {
                // Perform in-band key renegotiation:
                // 1. Server initiates new Kyber KEM with client's existing Kyber PK
                // 2. Derive fresh session keys from new shared secret
                // 3. Both sides switch to new keys atomically
                //
                // For simplicity, we close the connection and let client reconnect.
                // Full in-band renegotiation would require:
                // - RpcRequest::Renegotiate message type
                // - New Kyber encapsulation
                // - Synchronized key switch
                //
                // Current approach: force reconnect for forward secrecy
                println!("   Key renegotiation triggered (nonce counter high), closing connection");
                break;
            }
    ...
    async fn process_request(&self, request: RpcRequest) -> RpcResponse {
        match request {
            ...
            RpcRequest::SubmitStealthHint { hint_hex } => {
                ...
                // Limit hint size to prevent memory exhaustion (DoS protection)
                // Typical stealth hint is ~1KB, max reasonable is 64KB
                const MAX_HINT_SIZE: usize = 64 * 1024;
                if hint_bytes.len() > MAX_HINT_SIZE {
                    return RpcResponse::Error {
                        code: 400,
                        message: format!("Hint too large: {} bytes (max {})", hint_bytes.len(), MAX_HINT_SIZE),
                        data: None,
                    };
                }
...
async fn key_rotation_task(server: Arc<SecureRpcServer>) {
    // Key rotation uses a staged approach:
    // 1. Generate new identity (Falcon + Kyber keys)
    // 2. Announce new keys via next_key_announcement in ServerHello
    // 3. Wait for KEY_ROTATION_INTERVAL / 2 (grace period)
    // 4. Activate new identity as primary
    // 5. Old identity is discarded (zeroized)
    //
    // This allows clients to learn about new keys before they become active.
    ...
    loop {
        ...
        if last.elapsed() >= KEY_ROTATION_INTERVAL {
            // Check if we have a staged identity ready to activate
            let should_activate = {
                let active_at = server.next_identity_active_at.read().await;
                let next_id = server.next_identity.read().await;
                if let (Some(ts), Some(_)) = (*active_at, next_id.as_ref()) {
            ...
            if should_activate {
                // Activate staged identity
                let new_identity = {
                    let mut next = server.next_identity.write().await;
                    next.take()
                };
                
                if let Some(new_id) = new_identity {
                    let mut identity = server.identity.write().await;
                    println!("🔑 Key rotation: activating new identity");
                    println!("   Old Node ID: {}...", hex::encode(&identity.node_id[..8]));
                    println!("   New Node ID: {}...", hex::encode(&new_id.node_id[..8]));
                    *identity = new_id;
                    
                    // Clear activation timestamp
                    let mut active_at = server.next_identity_active_at.write().await;
                    *active_at = None;
                }
            } else {
                // Generate new staged identity for next rotation
                let new_identity = create_secure_rpc_identity()
                    .expect("Failed to generate new identity for rotation");
                
                let activation_time = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap()
                    .as_secs() + (KEY_ROTATION_INTERVAL.as_secs() / 2);
                
                println!("🔑 Key rotation: staging new identity (active in {} hours)",
                    KEY_ROTATION_INTERVAL.as_secs() / 2 / 3600);
                println!("   Next Node ID: {}...", hex::encode(&new_identity.node_id[..8]));
                
                {
                    let mut next = server.next_identity.write().await;
                    *next = Some(new_identity);
                }
                {
                    let mut active_at = server.next_identity_active_at.write().await;
                    *active_at = Some(activation_time);
                }
            }
