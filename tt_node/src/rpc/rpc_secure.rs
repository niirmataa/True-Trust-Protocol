...
impl SecureRpcServer {
    ...
    async fn process_request(&self, request: RpcRequest) -> RpcResponse {
        match request {
            ...
            RpcRequest::GetBalance { address_hex } => {
                // TODO: Implement balance tracking in NodeCore
                RpcResponse::Balance {
                    address_hex,
                    confirmed: 0,
                    pending: 0,
            ...
            RpcRequest::SubmitSignedStarkTx { tx_hex } => {
                // TODO: Implement STARK transaction submission
                match hex::decode(&tx_hex) {
                    Ok(tx_bytes) => {
                        // For now, submit as regular transaction
                        match self.node.submit_transaction(&tx_bytes).await {
    ...
    async fn cleanup_sessions_task(&self) {
        loop {
            ...
            if removed > 0 {
                println!("🧹 Cleaned up {} expired sessions ({} active)", removed, sessions.len());
            }
        }
    }
