...
pub fn verify_spend_authorization<C: PqcVerificationContext>(
    ...
    let note = ctx.load_note(nullifier).with_context(|| {
        format!(
            "Failed to load note for nullifier {}",
            hex::encode(nullifier)
        )
    })?;
...
pub fn verify_batch_spend_authorization<C: PqcVerificationContext>(
    ...
    for (nullifier, fp, sig) in spends {
        verify_spend_authorization(ctx, nullifier, fp, sig).with_context(|| {
            format!(
                "Failed to verify spend for nullifier {}",
                hex::encode(nullifier)
            )
        })?;
    }
...
pub fn compute_pqc_fingerprint(falcon_pk: &[u8], mlkem_pk: &[u8]) -> Hash32 {
...
mod tests {
    ...
    fn test_verify_spend_authorization() {
        ...
        assert!(
            ctx.is_nullifier_spent(&nullifier).unwrap(),
