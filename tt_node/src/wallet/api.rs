...
use crate::wallet::wallet_cli::{Keyset, load_wallet_file, decrypt_wallet_v3, WalletHeader};
...
pub fn load_wallet_keyset(wallet_path: &PathBuf, password: Option<String>) -> Result<(Keyset, WalletHeader)> {
    let wf = load_wallet_file(wallet_path)
        .context("Failed to load wallet file")?;
    ...
    let ks = Keyset::from_payload_v3(&secret)
        .context("Failed to parse wallet keys")?;
    
    Ok((ks, wf.header))
}

/// Get Falcon keys from wallet
pub fn get_falcon_keys_from_wallet(wallet_path: &PathBuf, password: Option<String>) -> Result<(FalconPublicKey, FalconSecretKey)> {
    let (keyset, _) = load_wallet_keyset(wallet_path, password)?;
    Ok((keyset.falcon_pk, keyset.falcon_sk))
}

/// Get Kyber keys from wallet
pub fn get_kyber_keys_from_wallet(wallet_path: &PathBuf, password: Option<String>) -> Result<(KyberPublicKey, KyberSecretKey)> {
    let (keyset, _) = load_wallet_keyset(wallet_path, password)?;
    Ok((keyset.mlkem_pk, keyset.mlkem_sk))
}

/// Get all keys from wallet
pub fn get_all_keys_from_wallet(wallet_path: &PathBuf, password: Option<String>) -> Result<(FalconPublicKey, FalconSecretKey, KyberPublicKey, KyberSecretKey, NodeId)> {
    let (keyset, _) = load_wallet_keyset(wallet_path, password)?;
    let node_id = node_id_from_falcon_pk(&keyset.falcon_pk);
    Ok((keyset.falcon_pk, keyset.falcon_sk, keyset.mlkem_pk, keyset.mlkem_sk, node_id))
}
