...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Info { crypto } => show_node_info(crypto),
    }
}
...
fn run_validator(
    ...
    rt.block_on(async {
        ...
        let (falcon_pk, falcon_sk) = if let Some(ks) = keystore {
            ...
            // TODO: Load from keystore (encrypted, Argon2, etc.)
        ...
        // Register as validator in consensus (demo-local)
        let mut consensus = consensus_pro::ConsensusPro::new_default();
        consensus.register_validator(node_id, stake)
            .expect("Failed to register validator: stake limits exceeded");
...
fn run_consensus_demo(validators: u32, rounds: u32) -> Result<()> {
    ...
    for i in 0..validators {
        ...
        let stake = 1_000_000 * (i as u128 + 1); // Variable stakes
        let quality = 0.8 + (i as f64 * 0.05); // Variable quality

        consensus.register_validator(id, stake)
            .expect("Failed to register validator in consensus demo");
    ...
    for slot in 0..rounds as u64 {
        ...
        let leader =
            consensus.select_leader(beacon).ok_or_else(|| anyhow!("No leader selected"))?;
        let leader_info = validator_ids
            .iter()
...
fn run_benchmarks(filter: Option<String>) -> Result<()> {
    ...
    if filter.as_ref().map_or(true, |f| f.contains("kyber")) {
        ...
        println!(
            ...
            KemSharedSecret::as_bytes(&ss1).len()
        );
        println!();
    }
    ...
    // Benchmark STARK (Winterfell range proof + Poseidon)
    if filter.as_ref().map_or(true, |f| f.contains("stark")) {
...
fn run_test_all() -> Result<()> {
    ...
    let (pk, _) = falcon_sigs::falcon_keypair();
    let id = node_id::node_id_from_falcon_pk(&pk);
    consensus.register_validator(id, 1_000_000)
        .expect("Failed to register validator in show_node_info");
    consensus.update_all_trust();
    let beacon = [0u8; 32];
    let _ = consensus
        .select_leader(beacon)
        .ok_or_else(|| anyhow!("No leader selected"))?;
    println!("✅ Consensus OK");

    // Test storage
    println!("Testing chain store...");
    let _store = chain_store::ChainStore::new();
