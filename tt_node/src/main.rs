...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::ConsensusDemo { validators, rounds } => run_consensus_demo(validators, rounds),
...
fn run_benchmarks(filter: Option<String>) -> Result<()> {
    ...
    if filter.as_ref().map_or(true, |f| f.contains("kyber")) {
        ...
        println!(
            "  Public key size: {} bytes",
            KemPublicKey::as_bytes(&pk).len()
        );
        println!(
            "  Ciphertext size: {} bytes",
            KemCiphertext::as_bytes(&ct).len()
        );
        println!(
