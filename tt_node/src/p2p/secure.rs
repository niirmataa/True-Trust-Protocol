...
/// Zeroize secret keys on drop to prevent memory leaks of sensitive data.
/// Note: pqcrypto types don't implement Zeroize, so we overwrite the bytes manually.
impl Drop for SecureNodeIdentity {
    fn drop(&mut self) {
        // Zero out secret key bytes by re-generating dummy keys
        // This overwrites the memory with fresh random data
        let (_, dummy_falcon_sk) = falcon_keypair();
        let (_, dummy_kyber_sk) = kyber_keypair();
        self.falcon_sk = dummy_falcon_sk;
        self.kyber_sk = dummy_kyber_sk;
        // NodeId is public, no need to zeroize
    }
}
...
pub fn handle_client_hello(
    ...
    // Verify node_id matches the fingerprint of provided keys
    let expected_node_id = compute_pqc_fingerprint(&client_falcon_pk, client_kyber_pk.as_bytes());
    if ch.node_id != expected_node_id {
        return Err(anyhow!(
            "ClientHello node_id mismatch: claimed {} but fingerprint is {}",
            hex::encode(ch.node_id),
            hex::encode(expected_node_id)
        ));
    }
...
pub fn handle_server_hello(
    ...
    // Parse and verify server's Falcon public key
    let server_falcon_pk =
        falcon_pk_from_bytes(&sh.falcon_pk).context("invalid server Falcon pk")?;
    
    // ServerHello doesn't contain kyber_pk directly, but we can verify node_id
    // against the falcon_pk (partial verification - full verification requires kyber_pk)
    // Note: Server's kyber_pk is not in ServerHello, so we can only verify Falcon part
    // The Falcon signature over the transcript provides authenticity guarantee
    ...
    // Weryfikacja podpisu serwera (this proves server owns the falcon_sk matching falcon_pk)
