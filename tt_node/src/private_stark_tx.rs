...
impl PrivateStarkTx {
    ...
    pub fn signing_message(&self) -> Vec<u8> {
        ...
        let mut msg = Vec::with_capacity(
            48 + 60 + self.recipient_stealth.size() + self.amount.size() + 24
        );
...
pub fn scan_sender_change(
    ...
    let (expected_key, expected_tag) = SenderChangeOutput::recover(our_kyber_sk, output.derivation_nonce);
...
mod tests {
    ...
    fn test_signing_message_includes_all_ciphertexts() {
        ...
        // sender_change: 32 + 8 + 8 = 48B
        // encrypted_sender_id: 12 + 48 = 60B
        // recipient_stealth: 8 + 32 + 8 + 8 + 1088 = 1144B (with scan_hint!)
        // amount: 16 + 100 + 200 = 316B
        // fee + nonce: 8 + 16 = 24B
        // TOTAL: 48 + 60 + 1144 + 316 + 24 = 1592B
        
        let expected_size = 48 + 60 + 1144 + 316 + 24;
        assert_eq!(msg.len(), expected_size, "signing message size mismatch");

        // Verify encrypted_sender_id is in message (offset 48)
        assert_eq!(&msg[48..60], &[3u8; 12], "encrypted_sender_id.nonce missing");
        assert_eq!(&msg[60..108], &[4u8; 48], "encrypted_sender_id.ciphertext missing");

        // Verify scan_hint is in message (offset 108)
        assert_eq!(&msg[108..116], &[10u8; 8], "recipient_stealth.scan_hint missing");

        // Verify stealth_key is in message (offset 116)
        assert_eq!(&msg[116..148], &[5u8; 32], "recipient_stealth.stealth_key missing");

        // Verify view_tag is in message (offset 148)
        assert_eq!(&msg[148..156], &[6u8; 8], "recipient_stealth.view_tag missing");

        // Verify hint_fingerprint is in message (offset 156)
        assert_eq!(&msg[156..164], &[9u8; 8], "recipient_stealth.hint_fingerprint missing");

        // Verify kem_ct is in message (offset 164)
        assert_eq!(&msg[164..174], &[7u8; 10], "recipient_stealth.kem_ct missing");

        // Verify range_proof is in message
        let range_proof_start = 108 + 1144 + 16; // after recipient_stealth + commitment
