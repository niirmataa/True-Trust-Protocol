...
impl RandomXVM {
    ...
    pub fn execute(&mut self, program: &RandomXProgram, dataset: &RandomXDataset) -> [u8; 32] {
    ...
    fn execute_instruction(&mut self, inst: &Instruction, dataset: &RandomXDataset) {
        match inst.opcode {
            ...
            Opcode::IADD_RS => {
                ...
                self.r[inst.dst as usize] =
                    self.r[inst.dst as usize].wrapping_add(src << (inst.imm & 3));
            }
            Opcode::ISUB_R => {
                let src = self.r[inst.src as usize];
            ...
            Opcode::IROR_R => {
                let src = self.r[inst.src as usize];
                self.r[inst.dst as usize] =
                    self.r[inst.dst as usize].rotate_right((src & 63) as u32);
            }
...
mod tests {
    ...
    fn test_program_generation() {
        ...
        assert_eq!(
            ...
            program2.instructions[0].opcode
        );
    }
