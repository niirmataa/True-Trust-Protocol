...
use tt_node::randomx_full::{mine_randomx, RandomXHasher};
...
fn mine_block(
    ...
    println!(
        "Difficulty: ~{:.0} hashes",
        compute_difficulty(&config.difficulty_target)
    );
    ...
    match result {
        ...
        None => Err(anyhow!(
            "Mining failed: no solution found in {} iterations",
            config.max_iterations
        )),
    }
}
...
/// Verify mined block
fn verify_block(hasher: &RandomXHasher, block: &MinedBlock, target: &[u8; 32]) -> Result<()> {
...
fn run_mining_simulation() -> Result<()> {
    ...
    for (i, block) in mined_blocks.iter().enumerate() {
        ...
        let validator_name = validator_keys
            .iter()
        ...
        let weight = consensus
            .compute_validator_weight(&validator_id)
            .unwrap_or(0);
        let total_weight: u128 = validator_keys
            .iter()
    ...
    println!(
        "Chain height: {}",
        mined_blocks.last().map(|b| b.header.height).unwrap_or(0)
    );
