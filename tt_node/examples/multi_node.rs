...
//!
//! This runs 3 nodes that connect to each other,
...
impl SimpleNode {
    ...
    async fn start(self: Arc<Self>) -> Result<()> {
        ...
        println!("[{}] Node started on port {}", self.config.name, self.config.port);
        println!("[{}] Node ID: {}", self.config.name, hex::encode(&self.node_id()[..8]));
    ...
    async fn handle_message(&self, peer_id: Hash32, msg: P2PMessage) -> Result<()> {
        match msg {
            ...
            P2PMessage::NewBlock { block } => {
                println!("[{}] Received new block at height {}",
                    self.config.name,
                    block.header.height
                );
...
async fn main() -> Result<()> {
    println!("╔══════════════════════════════════════════════════════════╗");
    println!("║  TRUE_TRUST Multi-Node Network Test                     ║");
    println!("╚══════════════════════════════════════════════════════════╝");
    println!();
    
    // Create 3 nodes
    println!("🚀 Creating 3 nodes...");
    
    let (pk1, sk1) = falcon_keypair();
    let (pk2, sk2) = falcon_keypair();
    let (pk3, sk3) = falcon_keypair();
    
    let node1 = Arc::new(SimpleNode::new(NodeConfig {
    ...
    let node2 = Arc::new(SimpleNode::new(NodeConfig {
        name: "Bob".to_string(),
        port: 9002,
        pk: pk2.clone(),
        sk: sk2,
    ...
    let node3 = Arc::new(SimpleNode::new(NodeConfig {
