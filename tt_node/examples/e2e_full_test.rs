...
fn main() {
    ...
    let alice_id = tt_node::p2p::secure::NodeIdentity::from_keys(
        alice_falcon_pk,
        alice_falcon_sk,
        alice_kyber_pk,
        alice_kyber_sk,
    ...
    let bob_id = tt_node::p2p::secure::NodeIdentity::from_keys(
        bob_falcon_pk,
        bob_falcon_sk,
        bob_kyber_pk,
        bob_kyber_sk,
    ...
    let recipient =
        tt_node::core::bytes32(tt_node::falcon_sigs::falcon_pk_to_bytes(&bob_id.falcon_pk));
    let output =
        tt_node::tx_stark::TxOutputStark::new(100_000, &blinding, recipient, &bob_id.kyber_pk);
    assert!(output.verify());
    println!("  âś… TxOutputStark utworzony (100,000 tokens)");

    let tx = tt_node::tx_stark::TransactionStark {
        inputs: vec![],
        outputs: vec![output.clone()],
        fee: 0,
        nonce: 1,
        timestamp: tt_node::core::now_ts(),
    };
    let (valid, total) = tx.verify_all_proofs();
    assert_eq!(valid, total);
    println!(
        "  âś… Transakcja zweryfikowana ({}/{} proofs OK)",
        valid, total
    );
    let tx_bytes = tx.to_bytes();

    println!("\nđź¤ť PQC Handshake (Kyber + Falcon)...");
    let (ch, transcript_client0) =
        tt_node::p2p::secure::build_client_hello(&alice_id, tt_node::p2p::secure::PROTOCOL_VERSION)
            .expect("CH failed");
    let (sh, session_key_server, transcript_server1) = tt_node::p2p::secure::handle_client_hello(
        &bob_id,
        &ch,
        tt_node::p2p::secure::PROTOCOL_VERSION,
        transcript_client0.clone_state(),
    )
    .expect("SH failed");
    let (session_key_client, transcript_client1) = tt_node::p2p::secure::handle_server_hello(
        &alice_id,
        &ch,
        &sh,
        transcript_client0,
    ...
    .expect("handle SH failed");
    let (cf, _) = tt_node::p2p::secure::build_client_finished(&alice_id, transcript_client1)
        .expect("CF failed");
    let _ = tt_node::p2p::secure::verify_client_finished(&ch.falcon_pk, transcript_server1, &cf)
        .expect("verify CF failed");
    ...
    let ciphertext = chan_alice
        .encrypt(&tx_bytes, b"TX_STARK_PQC")
        .expect("encrypt failed");
    let decrypted = chan_bob
        .decrypt(&ciphertext, b"TX_STARK_PQC")
        .expect("decrypt failed");
    assert_eq!(decrypted, tx_bytes);

    println!("\nđź’° Bob weryfikuje i odszyfrowuje wartoĹ›Ä‡...");
    let rx_tx =
        tt_node::tx_stark::TransactionStark::from_bytes(&decrypted).expect("TX deser failed");
    let (valid2, total2) = rx_tx.verify_all_proofs();
    assert_eq!(valid2, total2);
    let value = rx_tx.outputs[0]
        .decrypt_and_verify(&bob_id.kyber_sk)
        .expect("decrypt failed");
